...
int main(void)
{
  ...
  while(1) {
    ...
    uint8_t passbad = 0;
    // To fancy timing attacks (and since rand() is strictly deterministic)
    int wait = 0;

    for(uint8_t i = 0; i < sizeof(correct_passwd); i++){
        wait += (uint8_t)passwd[i];
    }
    wait = wait*17 % 10;
    
    trigger_high();

    // Stop fancy timing attack (Exercise part 3)
...
//     // Stop them fancy timing attacks (Exercise part 2)
//     for(volatile int i = 0; i < wait; i++){
//         ;
//     }
    delay_2_ms();
